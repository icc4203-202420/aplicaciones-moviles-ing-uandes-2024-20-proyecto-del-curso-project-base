...
import { Typography, List, ListItem, ListItemText, ListItemAvatar, Avatar, CircularProgress, Card, CardActions, CardContent, Button, Dialog, DialogContent } from '@mui/material';
...
function EventPopup({ open, onClose, event, onCheckIn }) {
  ...
  const handleViewGallery = () => {
    setShowGallery(true); // Mostrar la galería de imágenes
  };

  const handleCloseGallery = () => {
    setShowGallery(false); // Ocultar la galería de imágenes
  };
  ...
  return (
    <>
      ...
      {/* Galería de imágenes */}
      {event && showGallery && (
        <Dialog open={showGallery} onClose={handleCloseGallery} maxWidth="md" fullWidth>
          <DialogContent>
            <EventGallery eventId={event.id} />
            <Button onClick={handleCloseGallery} color="primary" variant="contained" sx={{ marginTop: 2 }}>
              Cerrar galería
            </Button>
          </DialogContent>
        </Dialog>
